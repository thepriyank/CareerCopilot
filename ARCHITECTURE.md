...
- **Explicitly out of scope (except the MVP exception below):** directly automating or scraping Naukri, Indeed, LinkedIn, Wellfound/AngelList, or storing a user's credentials for any of them — none offer a public self-serve jobs API for that (Indeed's Publisher program closed to new applicants in 2023), and doing so would mean either violating their ToS or storing user credentials. Storing a user's own third-party credentials remains a hard no per `CLAUDE.md` §7, unaffected by the exception below. Manual JD paste remains the supported path when no aggregator surfaces a given posting.
- **MVP exception — local JobSpy scraper (added 2026-09-12):** Adzuna and Jooble were dropped from live discovery (see `providers/index.ts`'s dated comment) after confirming both hard-truncate `description` to a short snippet, unusable for skill matching — the app's core feature. TheirStack does return full descriptions but its free tier (200 jobs/month) doesn't clear a pre-revenue MVP's cost bar. Explicit product decision: run [JobSpy](https://github.com/speedyapply/JobSpy) (scrapes Indeed/Naukri by default, India-only) as a **separate, local/self-hosted script** — `scripts/jobspy-ingest/` — posting into the pool via `POST /api/internal/jobs/ingest` (bearer-token-gated, `discoveryService.ts`'s `ingestExternalJobs`, same filter/dedup/skill-extraction path as every other source). This deliberately does NOT run on shared platform infrastructure (not in the backend, not in the Cloud Run discovery job) and is scoped to India only. This is an accepted MVP-stage tradeoff, not a reversal of the policy above — revisit before scaling or once revenue makes a licensed full-text source (TheirStack or similar) affordable. See that script's `README.md` for the full rationale and setup.
