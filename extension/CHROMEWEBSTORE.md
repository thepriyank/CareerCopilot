...
> Last Updated: 2026-09-23 (0.1.2 packaged)
...
| 0.1.2 | 2026-09-23 | **User-facing error messages only.** The popup never shows raw error text any more: network failures (browser "Failed to fetch") and non-JSON server responses get friendly copy instead of the error string or "Request failed: <status>", and chrome.scripting/DOM errors show "Something went wrong. Reload the page and try again." rather than browser-internal detail. Pairs with the backend change that only ever returns user-safe messages. No permission, manifest (besides version) or data-use changes — the privacy disclosures above still hold. Verified: 38/38 tests, `tsc --noEmit`, built bundle only embeds `api.jobmagnate.com` / `jobmagnate.com`. Packaged `jobmagnate-assisted-apply-0.1.2.zip` (forward-slash paths; the hand-made 0.1.0/0.1.1 zips used backslashes). | Ready to upload (Package tab) |
